...
from motores.constants import BOLETA_MAX, BOLETA_MIN, METODO_EFECTIVO, METODO_TRANSFERENCIA, VENDEDOR_LOCAL
...
def _resolver_duplicados_globales(filas: list[dict]) -> set[int]:
    ...
    for fila in filas:
        for part in _dividir_boletas(fila.get("boletas", "")):
            if part.isdigit() and BOLETA_MIN <= int(part) <= BOLETA_MAX:
                conteo[int(part)] += 1
...
def _validar_filas_vendedor(filas: list[dict], valor_boleta: int, vendedor_id: str, duplicados: set[int]) -> list[dict]:
    ...
    for fila in filas:
        ...
        partes = [p for p in _dividir_boletas(boletas_raw) if p.isdigit()]
        no_digitos = [p for p in _dividir_boletas(boletas_raw) if not p.isdigit()]
        if no_digitos:
            res["boletas"].append(f"Valores no num\u00e9ricos: {', '.join(no_digitos[:8])}.")
        fuera_rango = [p for p in partes if not (BOLETA_MIN <= int(p) <= BOLETA_MAX)]
        if fuera_rango:
            res["boletas"].append(f"Boleta(s) fuera del rango 0000-9999: {', '.join(f'#{p}' for p in fuera_rango[:8])}.")
        duplicadas_fila = {int(p) for p in partes if BOLETA_MIN <= int(p) <= BOLETA_MAX and int(p) in duplicados}
        if duplicadas_fila:
            res["boletas"].append("Boleta(s) duplicada(s) en el abono: " + ", ".join(f"#{n:04d}" for n in sorted(duplicadas_fila)) + ".")
        partes = [p for p in partes if BOLETA_MIN <= int(p) <= BOLETA_MAX]

        if not partes and not no_digitos:
...
def _validar_filas_cliente(filas: list[dict], valor_boleta: int, campo_errores: dict, duplicados: set[int]) -> list[dict]:
    ...
    for fila in filas:
        ...
        if not raw:
            ...
            num = int(raw)
            if not (BOLETA_MIN <= num <= BOLETA_MAX):
                res["boletas"].append(f"#{num:04d} est\u00e1 fuera del rango 0000-9999.")
            elif num in duplicados:
...
def _verificar_boletas_en_db(filas: list[dict], resultados: list[dict], tipo: str, vendedor_id: str = "", duplicados: set[int] | None = None) -> None:
    ...
    for fila in filas:
        for part in _dividir_boletas(fila.get("boletas", "")):
            if part.isdigit() and BOLETA_MIN <= int(part) <= BOLETA_MAX and int(part) not in duplicados:
                ids.add(int(part))
    if not ids:
        return
    ...
    for fila, res in zip(filas, resultados, strict=False):
        for part in _dividir_boletas(fila.get("boletas", "")):
            if not (part.isdigit() and BOLETA_MIN <= int(part) <= BOLETA_MAX):
                continue
            num = int(part)
